...
static char *parser_ini_str_trim_r(char *buf)
{
    ...
    y = kzalloc(len+1, GFP_KERNEL);//yang
        if (ERR_ALLOC_MEM(y)) {
...
static int allnode_peak_to_peak_cdc_data(int index)
{
    ...
    if (frm_buf == NULL) {
            ...
            frm_buf = (s32 **)kzalloc(tItems[index].bch_mrk_frm_num * 4, GFP_KERNEL);//yang
            ...
            for (i = 0 ; i < tItems[index].bch_mrk_frm_num ; i++) {
                ...
                frm_buf[i] = (s32*)kzalloc(core_mp.frame_len * 4, GFP_KERNEL);//yang
                if (ERR_ALLOC_MEM(frm_buf)) {
	...
	if (tItems[index].catalog == TX_RX_DELTA) {
		...
		if (tItems[index].spec_option == BENCHMARK) {
			...
			if (tItems[index].bch_mrk_min == NULL) {
				tItems[index].bch_mrk_max = (s32 **)kzalloc(tItems[index].bch_mrk_frm_num * 4, GFP_KERNEL);
...
static int open_test_sp(int index)
{
	...
	open = (struct mp_test_P540_open *)vmalloc(sizeof(struct mp_test_P540_open) * tItems[index].frame_count);
	if (ERR_ALLOC_MEM(open)) {
		ILI_ERR("Failed to allocate open data buffer \n");
		ret = -EMP_NOMEM;
		goto out;
	}
	ret = create_mp_test_frame_buffer(index, tItems[index].frame_count);
...
static int open_test_cap(int index)
{
	...
	open = (struct mp_test_open_c *)vmalloc(sizeof(struct mp_test_open_c) * tItems[index].frame_count);
	if (ERR_ALLOC_MEM(open)) {
		ILI_ERR("Failed to allocate open data buffer \n");
		ret = -EMP_NOMEM;
		goto out;
