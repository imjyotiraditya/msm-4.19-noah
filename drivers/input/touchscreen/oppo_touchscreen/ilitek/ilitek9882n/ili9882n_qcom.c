...
struct spi_transfer xfer_n[DMA_TRANSFER_MAX_CHUNK];
		...
		void *rxbuf, unsigned n_rx)
{
	...
	case SPI_WRITE:
		...
		for (xfercnt = 0; xfercnt < xferloop; xfercnt++) {
			...
			xfer_n[xfercnt].len = xferlen;
			xfer_n[xfercnt].tx_buf = ilits->spi_tx + xfercnt * DMA_TRANSFER_MAX_LEN;
			spi_message_add_tail(&xfer_n[xfercnt], &message);
	...
	case SPI_READ:
		...
		for (xfercnt = 0; xfercnt < xferloop; xfercnt++) {
			...
			xfer_n[xfercnt].len = xferlen;
			xfer_n[xfercnt].tx_buf = ilits->spi_tx;
			xfer_n[xfercnt].rx_buf = ilits->spi_rx + xfercnt * DMA_TRANSFER_MAX_LEN;
			spi_message_add_tail(&xfer_n[xfercnt], &message);
...
int ili_touch_esd_gesture_iram(void)
{
	...
	if (ilits->chip->core_ver < CORE_VER_1460) {
		...
		ges_run = SPI_ESD_GESTURE_RUN;//0xA67C9DFE
		pwd_len = 4;
	}
				//0xF38A94EF, answer = 0xA67C9DFE
